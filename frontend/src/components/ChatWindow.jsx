...
const CHAT_PREFILL_KEY = "ops.chatPrefill.v1";
...
function readPrefillFromSession() {
  try {
    const v = sessionStorage.getItem(CHAT_PREFILL_KEY);
    if (v) sessionStorage.removeItem(CHAT_PREFILL_KEY);
    return v || "";
  } catch {
    return "";
  }
...
export default function ChatWindow({ ops, onSelectOrder, onSelectRoute, onOpsPatch, orderMap, routeMap }) {
  ...
  // Prefill draft from:
  // 1) sessionStorage (cross-page actions)
  // 2) URL param ?prompt=...
  useEffect(() => {
    const sp = new URLSearchParams(location.search || "");
    const promptParam = (sp.get("prompt") || "").trim();
    const sessionPrefill = (readPrefillFromSession() || "").trim();

    const next = sessionPrefill || promptParam;
    if (next && !draft.trim() && !loading) setDraft(next);
  ...
  const quickPrompts = useMemo(() => {
    ...
    return [
      `📌 ${order} için gecikme riski nedir? 3 aksiyon öner.`,
      `🧭 ${route} için kapasite sapması ve alternatif plan öner.`,
      "SLA kritik siparişleri önceliklendir ve triage planı çıkar.",
      "Bugünkü sevkiyatlarda dar boğaz nerede? Operasyon planı üret.",
    ];
  }, [ops?.selectedOrderId, ops?.selectedRoute]);
        ...
        {loading && (
          <div className="w-full flex justify-start">
            <div className="max-w-[85%] md:max-w-[72%] rounded-2xl px-4 py-3 shadow-sm bg-[#F5F5DC] text-[#2B2B2B]">
              <div className="flex items-center gap-2 text-sm">
                <Loader2 size={16} className="animate-spin text-[#2D4F1E]" />
                <span className="text-[#2B2B2B]/80">Analiz ediliyor… SLA, risk ve rota sapmaları değerlendiriliyor</span>
              </div>
            </div>
          </div>
        )}
